...
void Tienda::mostrarInventarioCantMinima()
{
	int cantidadUsuario;
	cout << "Ingrese el valor minimo de productos: ";
	cin >> cantidadUsuario;
	for(map<int, Producto>::iterator pProducto = inventario.begin(); pProducto != inventario.end(); pProducto++)
	{
		Producto producto = pProducto->second;
		if(producto.getCantUnidades() < cantidadUsuario)
		{
			producto.mostrarProducto();
			cout << "cantidad: " << producto.getCantUnidades() << "\n";
		}
	}
}

//Falla por razones desconocidas
void Tienda::productosVendidosPorTipo()
{
	int cantSnacks = 0, cantLicor = 0;
	for(vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++)
	{
		//Factura factura = *pFactura;
		for(vector<DetalleFactura>::iterator pDetalle = pFactura->getDetalles().begin();
			pDetalle != pFactura->getDetalles().end(); pDetalle++)
		{
			/*
			Ingresa hasta este for pero al momento de intentar cualquier tipo de 
			proceso con la información de los detalles de factura el programa crashea,
			tanto si uso el modelo comentado abajo (DetalleFactura detalle = *pDetalle;)
			como si uso el modelo de pDetalle->
			*/
			//DetalleFactura detalle = *pDetalle;
			if(pDetalle->getProducto().getTipoProducto().getNombre() == "Snacks") 
			{
				cantSnacks += pDetalle->getCantUnidVend();
			} 
			else if(pDetalle->getProducto().getTipoProducto().getNombre() == "Licor")
			{
				cantLicor += pDetalle->getCantUnidVend();
			}
		}
	}
	cout << "Snacks vendidos: " << cantSnacks << "\n";
	cout << "Licores vendidos: " << cantLicor << "\n";
}

void Tienda::valorPagarIVADian()
{
	int total = 0;
	for(vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++)
	{
...
//Tambien crashea
void Tienda::gananciasGeneradas()
{
	int ganancia = 0;
	for(vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++)
	{
		for(vector<DetalleFactura>::iterator pDetalle = pFactura->getDetalles().begin(); 
			pDetalle != pFactura->getDetalles().end(); pDetalle++)
		{
			ganancia += (pDetalle->getProducto().getPrecio() - pDetalle->getProducto().getCosto()) * pDetalle->getCantUnidVend();
		}
	}
	cout << "Ganancias generadas: $" << ganancia << "\n";
}